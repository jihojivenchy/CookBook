...
//MARK: - DataMethod
    private func checkSquenceState() {
        ...
        if let state = UserDefaults.standard.string(forKey: "sequence") { //저장된 정렬 정보가 있으면 그것대로 정렬
            ...
            let query = db.collection("전체보기").whereField("user", isEqualTo: user.uid).order(by: "date", descending: true)
    ...
    private func sequencePressed(sequence : String) {
        ...
        case "과거순":
            let query = db.collection("전체보기").whereField("user", isEqualTo: user.uid).order(by: "date", descending: false)
            self.sequenceString = sequence
            getRecipeData(query: query)
            UserDefaults.standard.set(sequence, forKey: "sequence")
            
        default:
            let query = db.collection("전체보기").whereField("user", isEqualTo: user.uid).order(by: "heartPeople", descending: true)
            self.sequenceString = sequence
            getRecipeData(query: query)
            UserDefaults.standard.set(sequence, forKey: "sequence")
        }
...
extension MyRecipeViewController : UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        case UICollectionView.elementKindSectionHeader:
            ...
            headerView.categoryLabel.text = "나의 레시피"
