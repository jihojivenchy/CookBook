...
//MARK: - DataMethod
    ...
    private func getDataUpdate() {
        ...
        db.collection("전체보기").order(by: "date", descending: true).limit(to: 10).addSnapshotListener { querySnapshot, error in
            if let e = error {
    ...
    private func getNextPageData(){
        ...
        db.collection("전체보기").order(by: "date", descending: true).start(afterDocument: lastSnapshotDocumentId).limit(to: 10).addSnapshotListener { querySnapshot, error in
            if let e = error {
                print("Error find data : \(e)")
            }else{
...
extension AllViewController : UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout  {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.titleLable.text = self.userModel[indexPath.row].title
        cell.whoLabel.text = "작성자: \(self.userModel[indexPath.row].userName)"
        cell.difficultyLabel.text = "난이도: \(self.userModel[indexPath.row].segment)"
        cell.dateLabel.text = self.userModel[indexPath.row].saveDate
        cell.userUidLabel.text = self.userModel[indexPath.row].userUid
