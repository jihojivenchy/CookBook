...
class FuViewController : UIViewController {
    ...
    private func getUserBookUpdate() {
        ...
        db.collection("전체보기").whereField("tema", isEqualTo: "퓨전").order(by: "date", descending: true).limit(to: 10).addSnapshotListener { querySnapshot, error in
            if let e = error {
    ...
    private func getNextPageData(){
        ...
        db.collection("전체보기").whereField("tema", isEqualTo: "퓨전").order(by: "date", descending: true).start(afterDocument: lastSnapshotDocumentId).limit(to: 10).addSnapshotListener { querySnapshot, error in
            if let e = error {
                print("Error find data : \(e)")
            }else{
...
extension FuViewController : UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout  {
